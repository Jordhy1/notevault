  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-zinc-50 p-6 dark:bg-zinc-950">
      <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-8 dark:border-zinc-800 dark:bg-zinc-900">
        <h1 className="text-2xl font-semibold">Sign in to NoteVault</h1>
        <form className="mt-6 space-y-4" action={async (formData) => {
          ...
          <input name="password" type="password" required placeholder="Password" className="w-full rounded-lg border p-3 dark:border-zinc-700 dark:bg-zinc-950" />
          <button type="submit" className="w-full rounded-lg bg-zinc-950 px-4 py-3 font-medium text-white dark:bg-white dark:text-zinc-950">Sign in</button>
        </form>
        ...
        <p className="mt-6 text-sm text-zinc-500">No account? <Link href="/register" className="underline">Create one</Link></p>
      </div>
    </main>
  );
}
